  ...
  PageBlog: {
    ...
    Card: {
      ...
      actions: async (blog: any, _args: any, ctx: ApolloContext) => {
        const slug = createPath('blog', getLocalizedField(blog.fields, 'slug', ctx));
        console.log({ slug });
        const readMoreLink = createType('Link', {
          href: slug,
          text: 'Read more',
          variant: 'contained'
        });
      ...
      tags: async (blog: any, _args: any, ctx: ApolloContext) => {
        ...
        topicActions.__fieldName__ = 'topics';
        return topicActions;
      },
